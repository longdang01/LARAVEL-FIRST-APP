<!-- <!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="utf-8" />
        <meta http-equiv="X-UA-Compatible" content="IE=edge" />
        <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no" />
        <meta name="description" content="" />
        <meta name="author" content="" />
        <title>Product Management</title>
        
        <link href="https://fonts.googleapis.com/css2?family=Open+Sans:ital,wght@0,300;0,400;0,500;0,600;0,700;0,800;1,300;1,400;1,500;1,600;1,700;1,800&family=Roboto:ital,wght@0,300;0,400;0,500;0,700;0,900;1,300;1,400;1,500;1,700;1,900&display=swap" rel="stylesheet">
        <link href="/assets/admin/dist/libs/fontawesome/css/all.min.css">
        <link href="/assets/admin/dist/libs/bootstrap/css/bootstrap.min.css" rel="stylesheet" />
        <link href="/assets/admin/dist/css/styles.css" rel="stylesheet" />
    </head>
    <body ng-app="App" ng-controller="ProductController" class="sb-nav-fixed"> -->
@extends('_layout')
@section('content')        
        <div ng-app="App" ng-controller="ProductController">
            <div class="container">
                <div class="d-flex align-items-center justify-content-between py-3">
                    <h3 style="font-size: 1.4rem; font-weight: 600">Product API</h3>
                    <a href="#" class="btn btn-success" ng-click="showModal(0, product)">Create</a>
                </div>
                <div>
                    <div>
                        <input type="text" class="form-control" ng-model="keySearch">
                    </div>
                    <table style="font-size: .9rem;" class="table table-bordered">
                        <thead>
                            <tr>
                                <th>Serial</th>
                                <th>Picture</th>
                                <th>Name</th>
                                <th>Price</th>
                                <th>Edit</th>
                                <th>Delete</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr dir-paginate="product in products | itemsPerPage:pageSize | filter: { name: keySearch }"
                                current-page="currentPage">
                                <td>@{{ $index + currentPage * 10 - 9 }}</td>
                                <td><img ng-if="product.image" ng-src="/upload/sanpham/@{{product.image}}" style='width:100px'></td>
                                <td>@{{ product.name }}</td>
                                <td>@{{ product.unit_price | number:0 }}</td>
                                <td><a href="" ng-click="showModal(1, product)" class="btn btn-info">Edit</a></td>
                                <td><a href="" class="btn btn-danger" ng-click="delete(product)">Delete</a></td>
                            </tr>
                        </tbody>
                    </table>
                    <div class="d-flex justify-content-end">
                        <dir-pagination-controls 
                            max-size="10" 
                            direction-links="true"
                            boundary-links="true"
                            >
                        </dir-pagination-controls>
                    </div>
                    

                </div>
            </div>

            <!-- Modal -->
            <div class="modal fade" id="handleProduct">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">@{{ title }}</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div>
                                <div class="mb-3">
                                    <label class="form-label">Name</label>
                                    <input type="text" ng-model="product.name" class="form-control">
                                </div>
                                <!-- <div class="mb-3">
                                    <label class="form-label">Category</label>
                                    <select class="form-select">
                                        <option ng-repeat="category in categories" value="@{{ category.id }}">
                                            @{{ category.tenloai }}
                                        </option>
                                    </select>
                                </div> -->
                                <div class="mb-3">
                                    <label class="form-label">Price</label>
                                    <input type="text" ng-model="product.unit_price" class="form-control">
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                            <button type="button" class="btn btn-primary" ng-click="save();">Save</button>
                        </div>
                    </div>
                </div>
            </div>
            
        </div>

        <!-- <script src="/assets/admin/dist/libs/jquery/jquery-3.6.0.min.js"></script>
        <script src="/assets/admin/dist/libs/bootstrap/js/popper.min.js"></script>
        <script src="/assets/admin/dist/libs/bootstrap/js/bootstrap.min.js"></script>
        <script src="/assets/admin/dist/js/app.js"></script>
    </body>
    </html> -->
    <!-- <script src="/assets/admin/dist/libs/angularjs/angularjs.min.js"></script>
    <script src="/assets/admin/dist/libs/pagination/dirpagination.js"></script> -->
    <!-- <script src="/assets/admin/dist/controllers/ProductController.js"></script> -->
@stop